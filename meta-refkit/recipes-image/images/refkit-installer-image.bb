...
REFKIT_INSTALLER_UEFI_COMBO () {
    populate () {
        ...
        if [ "$uuid" ]; then
            ...
            # explicitly by setting the USE_TPM12 env variable to "true" or "false".
            # The default is "true" if there is a /dev/tpm* device.
            USE_TPM12=${USE_TPM12:-`ls /dev/tpm* >/dev/null 2>&1 && echo true || echo false`}
            ...
            USE_FIXED_PASSWORD=${USE_FIXED_PASSWORD:-false}

            if [ ! -s "$keyfile" ] &&
               $USE_FIXED_PASSWORD; then
